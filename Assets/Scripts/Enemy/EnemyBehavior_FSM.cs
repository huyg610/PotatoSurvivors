...
public partial class EnemyBehavior : MonoBehaviour
{
    ...
    private const float kSizeChangeFrames = 25f;
    private const float kRotateFrames = 25f;
    ...
    private const float kChaseDistance = 100f;
    ...
    private void ServiceCCWState()
    {
        if (mStateFrameTick > kRotateFrames)
        {
            ...
            // Attempt to capture the camera if we don't already have it
            if (!mHasCam)
                mHasCam = sEnemySystem.EnableEnemyCam(transform.position, mTarget.transform.position);
        }
        else
        {
            mStateFrameTick++;
    ...
    private void ServicePatrolState()
    {
        ...
        PointAtPosition(sWayPoints.GetWayPoint(mWayPointIndex), kTurnRate);
    ...
    private void ServiceChaseState()
    {
        ...
        if (Vector3.Distance(transform.position, mTarget.transform.position) < kChaseDistance)
        {
            ...
            if (mHasCam)
                sEnemySystem.UpdateEnemyCam(transform.position, mTarget.transform.position);
        } else
        {
            // done
