...
public partial class CameraSupport: MonoBehaviour
{
    ...
    public void ZoomToNewSize(float ns)
    {
        float s = mTheCamera.orthographicSize;
        float z = ns / s;
        Zoom(z);
    }
    ...
    {
        // Should not allow douelbe shake (position will ended up moving
        if (mShake.ShakeDone())
            mShake.SetShakeMagnitude(delta, transform.position);
    }

    public void SetLerpParameters(float period, float rate)
    {
        mPositionLerp.SetLerpParms(period, rate);
        mSizeLerp.SetLerpParms(period, rate);
    }
